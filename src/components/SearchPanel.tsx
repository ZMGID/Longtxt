...
export function SearchPanel({
  ...
  const copy = {
    zh: {
      jumpToTimeline: '跳转到时间轴',
      title: '搜索生成',
      description: '先把问题写清楚，再决定是只看相关块，还是继续生成一篇结构化文档。',
      browsingTag: '按标签浏览',
      clear: '清除',
      ...
      generating: '生成中…',
      generate: '生成文档',
      collapsedHint: '当前窗口较小，已自动收起生成区；放大窗口后可查看生成文档与参考块。',
      documentTitle: '生成文档',
      topic: '主题',
      documentStreaming: '正在流式生成',
      documentDone: '生成完成',
      documentError: '生成失败',
      copied: '已复制到剪贴板',
      copy: '复制全文',
      saveSnapshot: '保存快照',
      documentEmpty: '点击“生成文档”后，这里会逐段出现编排结果。',
      referencesTitle: '本次参考块',
      referencesHint: '生成完成后，这里会展示本次真正进入文档生成的引用块，并可继续沉淀到 notebook。',
      createNotebook: '新建 notebook',
      creatingNotebook: '新建中…',
      addToNotebook: '加入当前 notebook',
      addingToNotebook: '加入中…',
      saving: '保存中…',
      noReferences: '当前还没有可展示的参考块。生成失败或空文档时，这里不会保留上一轮的旧引用。',
      referencesLoading: '参考块加载中…',
      referenceMeta: '本次参考',
      referencesEmptyDone: '这次生成没有引用到已有块。你仍然可以保存快照，但暂时没有可沉淀的参考块集合。',
    },
    en: {
      jumpToTimeline: 'Jump to timeline',
      title: 'Search & Draft',
      description: 'Describe your intent clearly, then decide whether to inspect related blocks only or generate a structured document.',
      browsingTag: 'Browsing tag',
      clear: 'Clear',
      placeholder: 'Describe what you want to find, or the document topic you want to generate.',
      searching: 'Searching…',
      search: 'Search',
      generating: 'Generating…',
      generate: 'Generate document',
      collapsedHint: 'The window is currently small, so the document panel is collapsed. Enlarge the window to see generated content and references.',
      documentTitle: 'Generated document',
      topic: 'Topic',
      documentStreaming: 'Streaming',
      documentDone: 'Ready',
      documentError: 'Failed',
      copied: 'Copied to clipboard.',
      copy: 'Copy all',
      saveSnapshot: 'Save snapshot',
      documentEmpty: 'After you click “Generate document”, the drafted content will appear here progressively.',
      referencesTitle: 'References',
      referencesHint: 'After generation finishes, the blocks actually used in the document will appear here and can be deposited into a notebook.',
      createNotebook: 'New notebook',
      creatingNotebook: 'Creating…',
      addToNotebook: 'Add to current notebook',
      addingToNotebook: 'Adding…',
      saving: 'Saving…',
      noReferences: 'No references are available right now. Failed or empty generations do not keep references from the previous run.',
      referencesLoading: 'Loading references…',
      referenceMeta: 'Reference',
      referencesEmptyDone: 'This generation did not cite existing blocks. You can still save a snapshot, but there is no reference set to deposit yet.',
    },
  }[language]
                ...
                <SearchResultCard
                  ...
                  headerActions={(
                    <div className="flex items-center gap-1">
                      <button
                        type="button"
                        aria-label={copy.jumpToTimeline}
                        title={copy.jumpToTimeline}
                        onClick={() => {
  ...
  return (
    <div className="flex min-h-0 min-w-0 flex-1 flex-col gap-4 overflow-hidden">
      <div className="shrink-0 rounded-lg border border-stone-200 bg-white/70 p-4">
        <p className="mb-2 text-xs font-medium uppercase tracking-wider text-stone-400">{copy.title}</p>
        <p className="mb-3 text-xs leading-5 text-stone-500">{copy.description}</p>

        {browseTag ? (
          <div className="mb-3 flex items-center gap-2">
            <span className="rounded border border-stone-200 bg-stone-50 px-2.5 py-1 text-xs font-medium text-stone-600">{copy.browsingTag}: {browseTag}</span>
            <button type="button" onClick={onClearBrowseTag} className="text-xs text-stone-400 transition hover:text-stone-600">{copy.clear}</button>
          </div>
        ) : null}
        ...
        {!showDocumentPanel ? (
          <p className="mt-2 text-xs leading-5 text-stone-400">{copy.collapsedHint}</p>
        ) : null}
      </div>
          ...
          <section data-testid="search-document-panel" className="flex min-h-0 min-w-0 flex-col gap-3 overflow-hidden">
            <div className="flex shrink-0 items-center justify-between gap-3">
              <div className="min-w-0">
                <p className="text-xs font-medium uppercase tracking-wider text-stone-400">{copy.documentTitle}</p>
                {document.topic ? <p className="mt-0.5 truncate text-xs text-stone-500">{copy.topic}: {document.topic}</p> : null}
              </div>
              <div className="flex shrink-0 items-center gap-2">
                {document.status !== 'idle' ? (
                  <span className="text-xs text-stone-400">
                    {document.status === 'streaming' ? copy.documentStreaming : document.status === 'done' ? copy.documentDone : document.status === 'error' ? copy.documentError : ''}
                  </span>
                ) : null}
                {canCopyDocument ? (
                  <button type="button" onClick={() => { navigator.clipboard.writeText(document.content); toast('success', copy.copied) }} className="rounded border border-stone-200 px-3 py-1.5 text-xs font-medium text-stone-600 transition duration-150 hover:bg-stone-50 active:scale-[0.97]">{copy.copy}</button>
                ) : null}
                {document.status === 'done' && canCopyDocument ? (
                  <button type="button" onClick={onSaveSnapshot} className="rounded bg-stone-900 px-3 py-1.5 text-xs font-medium text-white transition hover:bg-stone-700">{copy.saveSnapshot}</button>
                ) : null}
              </div>
            </div>
            ...
            <div className="grid min-h-0 flex-1 gap-3 xl:grid-cols-[minmax(0,1fr)_minmax(17rem,20rem)]">
              <div data-testid="generated-document-scroll" className="min-h-0 overflow-y-auto overflow-x-hidden rounded-lg border border-stone-200 bg-white/70 px-5 py-5">
                {document.content ? (
                  ...
                  <p className="text-sm leading-7 text-stone-400">{copy.documentEmpty}</p>
                )}
              ...
              <aside className="flex min-h-0 flex-col gap-3 overflow-hidden rounded-lg border border-stone-200 bg-stone-50/70 px-4 py-4">
                <div className="shrink-0">
                  <p className="text-xs font-medium uppercase tracking-wider text-stone-400">{copy.referencesTitle}</p>
                  <p className="mt-1 text-xs leading-5 text-stone-500">{copy.referencesHint}</p>
                </div>
                ...
                <div className="shrink-0 flex flex-wrap gap-2">
                  ...
                  >
                    {documentDepositAction === 'create' ? copy.creatingNotebook : copy.createNotebook}
                  </button>
                  <button
                    type="button"
                  ...
                  >
                    {documentDepositAction === 'append'
                      ? copy.addingToNotebook
                      : selectedNotebook
                        ? `${copy.addToNotebook}「${selectedNotebook.title}」`
                        : copy.addToNotebook}
                  </button>
                </div>
                ...
                <div className="min-h-0 flex-1 overflow-y-auto pr-1">
                  {!canShowReferences ? (
                    ...
                    <div className="rounded-lg bg-white/80 px-4 py-4 text-sm text-stone-500">{copy.referencesLoading}</div>
                    ...
                    <div className="space-y-2">
                      {documentReferences.map((result) => (
                        <SearchResultCard
                          ...
                          onTagClick={onTagClick}
                          showScore={false}
                          metaLabel={copy.referenceMeta}
                        />
                      ))}
                    </div>
                  ) : (
                    <div className="rounded-lg border border-dashed border-stone-200 bg-white/80 px-4 py-4 text-sm leading-6 text-stone-500">
                      {copy.referencesEmptyDone}
                    </div>
                  )}
                </div>
              </aside>
