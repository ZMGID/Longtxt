...
export function TopBar({ blockCount, activeView, activePanel, aiStatusLabel, searchPreview, onSelectView, onOpenSearch, onOpenSettings }: TopBarProps) {
  return (
    <header className="rounded-[32px] border border-stone-200/70 bg-[#faf8f5]/75 px-6 py-5 shadow-[0_30px_80px_rgba(68,48,22,0.08)] backdrop-blur">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.34em] text-stone-400">Changbu MVP v1</p>
        ...
        <div className="flex flex-wrap items-center gap-3">
            ...
            {[
              ...
              <button
                ...
                className={`rounded-full px-4 py-2 text-sm font-medium transition ${
                  activeView === view ? 'bg-stone-900 text-white' : 'text-stone-700 hover:bg-[#faf8f5]'
                }`}
              >
                {label}
              </button>
          ...
          <button
            ...
            className={`flex min-w-[320px] items-center justify-between rounded-full border px-5 py-3 text-sm transition ${
              activePanel === 'search' ? 'border-stone-900 bg-[#faf8f5] text-stone-900 shadow-sm' : 'border-stone-200 bg-stone-50 text-stone-600 hover:bg-[#faf8f5]'
            }`}
          >
