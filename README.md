...
- 设置页彻底独立为单独窗口，主窗口去掉重复设置逻辑，整体结构更轻，双栏布局也更紧凑
- 新增每日回顾与 AI 洞察窗口，支持流式生成、快照保存，并把相关设置接入到统一配置里
- 外部接入升级为通用接入包，支持通过 CLI / Skill 读取、检索、创建、更新和删除笔记内容
- 数据管理继续补强：支持按天浏览与批量删除块，同时 JSON 完整备份现在会连同设置快照一起导出与恢复
- 继续优化日历、连接图、笔记本、文档快照等页面，减少套娃式卡片，并修复窄窗口下的布局与加载细节
