...
- 设置改为独立窗口，采用更紧凑的双栏结构，并把不常用参数收口到高级设置
- 新增数据管理页，可按日期浏览内容、批量删除、刷新统计、打开数据与设置目录，并补上备份 / 加载备份入口
- 日历、笔记本与文档快照界面继续瘦身，减少卡片套娃，改为更扁平的分割式布局
- 优化标签导出、热力图细节与缩放响应，修复窄窗口下的空白区、边框遮挡和刷新不一致
- 减少页面切换时的加载闪烁，切到笔记本、日历、搜索、快照等界面更直接
