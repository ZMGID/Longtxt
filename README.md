...
- 重做文档快照页，改为正文优先阅读与检索优先结构，压缩无用说明与标签信息，支持更紧凑的双栏浏览
- 数据管理页、设置页与快照页统一回到白色基底，缩小模式下的布局更稳定
- 修复 AI 洞察缓存失效窗口、设置文件跨进程并发写入覆盖，以及语言切换后时间轴 / 日历局部文案不刷新的问题
- 完善相关测试覆盖，补强回顾、窗口状态、事件桥与块事件查询路由的回归验证
